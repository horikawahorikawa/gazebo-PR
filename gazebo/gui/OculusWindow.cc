...
void OculusWindow::OculusCameraParameter(float FOV_angle){
  //this->oculusCamera->OculusFOV = FOV_angle;
  this->oculusCamera->SetOculusFOV(FOV_angle);
}
void OculusWindow::ConnectROStoVicon(){
  this->oculusCamera->ConnectROS();
}
void OculusWindow::OculusChangePose(float Input_roll, float Input_pitch, float Input_yaw)
{
  gazebo::math::Angle roll;
  roll.SetFromDegree((double)Input_roll);
  gazebo::math::Angle pitch;
  pitch.SetFromDegree((double)Input_pitch);
  gazebo::math::Angle yaw;
  yaw.SetFromDegree((double)Input_yaw);
  std::cout<<"add_rad"<<(float)yaw.Radian()<<std::endl;

  this->oculusCamera->OculusAddrpy((float)roll.Radian(), (float)pitch.Radian(), (float)yaw.Radian());
}
