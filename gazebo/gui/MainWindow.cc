...
#include "gazebo/gui/AdjustParameterDialog.hh"
#endif
  ...
  : renderWidget(0)
{
  ...
  this->adjustparameterdialog = NULL;
#endif
...
/////////////////////////////////////////////////
void MainWindow::OculusTool()
{
  if(!adjustparameterdialog){
  adjustparameterdialog = new AdjustParameterDialog;
  connect(adjustparameterdialog, SIGNAL(okButtonClicked() ), this, SLOT(setOculusParameter()) );
  }
  if(adjustparameterdialog->isHidden() ) {
    adjustparameterdialog->show();
  }else{
    adjustparameterdialog->activateWindow();
  }
}
void MainWindow::setOculusParameter()
{
    QString qstr0, qstr1, qstr2, qstr3;
  	adjustparameterdialog->getLineEditText(&qstr0, &qstr1, &qstr2, &qstr3);
    float InputFOV = qstr0.toFloat();
    float Input_roll = qstr1.toFloat();
    float Input_pitch = qstr2.toFloat();
    float Input_yaw = qstr3.toFloat();

    this->oculusWindow->OculusCameraParameter(InputFOV);
  	std::cout << InputFOV << std::endl;

    this->oculusWindow->OculusChangePose(Input_roll, Input_pitch, Input_yaw);
  	std::cout << Input_roll << std::endl;
    std::cout << Input_pitch << std::endl;
  	std::cout << Input_yaw << std::endl;
}
void MainWindow::ROSvicon()
{
  this->oculusWindow->ConnectROStoVicon();
}
/////////////////////////////////////////////////
...
void MainWindow::CreateActions()
{
  ...
  */
  ...
  g_OculusTool = new QAction(tr("SetCameraParameter"), this);
  g_OculusTool->setStatusTip(tr("Set Camera Parameter"));
  connect(g_OculusTool, SIGNAL(triggered()), this, SLOT(OculusTool()));

  g_ROSvicon = new QAction(tr("ROSconnectVicon"), this);
  g_ROSvicon->setStatusTip(tr("Connecting ROS"));
  connect(g_ROSvicon, SIGNAL(triggered()), this, SLOT(ROSvicon()));

#ifndef HAVE_OCULUS
  g_viewOculusAct->setEnabled(false);
  g_OculusTool->setEnabled(false);
  g_ROSvicon->setEnabled(false);
#endif
...
void MainWindow::CreateMenuBar()
{
  ...
  windowMenu->addSeparator();
  windowMenu->addAction(g_OculusTool);
  windowMenu->addAction(g_ROSvicon);
  windowMenu->addSeparator();
