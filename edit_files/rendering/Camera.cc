...
void Camera::Init()
{
  ...
  this->CreateCamera();
      ...
      Ogre::MaterialPtr left_cam_material = Ogre::MaterialManager::getSingleton().create("leftBackground", "General");
      left_cam_material->getTechnique(0)->getPass(0)->createTextureUnitState("test.jpg");
      left_cam_material->getTechnique(0)->getPass(0)->setDepthCheckEnabled(false);
      left_cam_material->getTechnique(0)->getPass(0)->setDepthWriteEnabled(false);
      left_cam_material->getTechnique(0)->getPass(0)->setLightingEnabled(false);

      // Create background rectangle covering the whole screen
      Ogre::Rectangle2D* left_rect = new Ogre::Rectangle2D(true);
      left_rect->setCorners(-1.0, 1.0, 1.0, -1.0);
      left_rect->setMaterial("leftBackground");

      // Render the background before everything else
      left_rect->setRenderQueueGroup(Ogre::RENDER_QUEUE_BACKGROUND);

      // Use infinite AAB to always stay visible
      Ogre::AxisAlignedBox aabInf;
      aabInf.setInfinite();
      left_rect->setBoundingBox(aabInf);

      // Attach background to the scene
      //SceneNode* node = mSceneMgr->getRootSceneNode()->createChildSceneNode("leftBackground");
      this->sceneNode->attachObject(left_rect);
  #endif
